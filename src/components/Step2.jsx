...
const Step2 = ({ formData, setFormData, category }) => {
    ...
    const [activeCategory, setActiveCategory] = useState("POCKETS");
    ...
    // Funkcija suskaičiuoti kiek pasirinkimų yra kiekvienoje kategorijoje
    const getCategorySelectionCount = (category) => {
        ...
        Object.keys(categoryItems).forEach((subCategory) => {
            count += selectedItems.filter((item) =>
                categoryItems[subCategory].some((catItem) => catItem.name === item.name)
            ).length;
        });
        return count;
    };
